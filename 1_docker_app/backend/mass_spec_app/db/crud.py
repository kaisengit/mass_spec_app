...
from mass_spec_app.scripts.chem_utils import (get_measured_formula,
                                              get_monoisotopic_mass)
from sqlalchemy.orm import Session


# Adduct CRUD
def get_adducts(db: Session, skip: int = 0, limit: int = 100) -> List[models.Adduct]:
...
# Compound CRUD
def create_compound(db: Session, compound: schemas.CompoundCreate) -> models.Compound:
    ...
    db_compound = models.Compound(
        ...
        molecular_formula=compound.molecular_formula,  # Use the computed molecular formula
        type=compound.type,
...
def get_or_create_retention_time(
    ...
    new_retention_time = models.RetentionTime(
        retention_time=retention_time.retention_time, comment=retention_time.comment
    )
...
def create_measured_compound_and_retention_time(
    ...
    if not adduct:
        raise ValueError(
            f"Adduct '{measured_compound.adduct_name}' not found in the database."
        )
    ...
    if not compound:
        raise ValueError(
            f"Compound '{measured_compound.compound_id}' not found in the database."
        )
        # retentiont ime
    ...
    # Calculate measured mass using the compound's mass and adduct's mass adjustment
    ...
    db_measured_compound = models.MeasuredCompound(
        ...
        adduct_id=adduct.adduct_id,  # Mapped from the adduct_name
        retention_time_id=retention_time_entry.retention_time_id,
        measured_mass=measured_mass,  # Computed measured mass
        molecular_formula=molecular_formula,  # Use the compound's molecular formula
    )
...
def get_adduct_by_id(db: Session, adduct_id: int) -> Optional[models.Adduct]:
    ...
    return db.query(models.Adduct).filter(models.Adduct.adduct_id == adduct_id).first()


# CRUD to Get a Single Compound by ID
def get_compound_by_id(db: Session, compound_id: int) -> Optional[models.Compound]:
...
def get_measured_compound_by_id(
    ...
    return (
        ...
        .filter(models.MeasuredCompound.measured_compound_id == measured_compound_id)
        .first()
    )
...
def get_measured_compounds_filtered(
    ...
    query = (
        ...
        .join(
            models.Adduct, models.MeasuredCompound.adduct_id == models.Adduct.adduct_id
        )
    )
    ...
    if retention_time is not None:
        ...
        query = query.filter(models.RetentionTime.retention_time == retention_time)
